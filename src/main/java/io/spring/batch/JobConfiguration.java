	    ...
	    return new ItemProcessor<Person, Student>() {
	        ...
	        public Student process(Person person) throws Exception {
	            ...
	            if (personExistsInPerson) {
	                // Check if the person exists in the "STUDENT" table
	                boolean personExistsInStudent = checkIfPersonExistsInStudentTable(person);

	                if (personExistsInStudent) {
	                    // If the person exists in "STUDENT," update the existing record
	                    ...
	                    // If the person doesn't exist in "STUDENT," create a new student
	                    Student student = new Student();
	                    student.setId1(person.getId1());
	                    student.setFirstName(person.getFirstName());
	                    student.setLastName(person.getLastName());
	                ...
	                // If the person doesn't exist in "PERSON," return null to skip adding it to "STUDENT"
	                return null;
	            }
	        }
	    };
...
//	            preparedStatement.setTimestamp(4, new Timestamp(person.getBirthdate().getTime()));
	            try (ResultSet resultSet = preparedStatement.executeQuery()) {
	                if (resultSet.next()) {
	                    int count = resultSet.getInt(1);
	                    return count > 0; // If count > 0, the person exists in the "STUDENT" table
	                }
	            }
	        }
	    ...
	    // If there was an error or the person was not found, return false
	    return false;
	}
...
//	            preparedStatement.setTimestamp(4, new Timestamp(person.getBirthdate().getTime()));
	            try (ResultSet resultSet = preparedStatement.executeQuery()) {
	                if (resultSet.next()) {
	                    int count = resultSet.getInt(1);
	                    return count > 0; // If count > 0, the person exists in the "PERSON" table
	                }
	            }
	        }
	    } catch (SQLException e) {
